...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        trainObj, top1 = train(train_loader, model, ema_model, criterion, optimizer, scheduler, epoch)
        valObj, prec1 = val(val_loader, model, ema_model, criterion)
...
def train(train_loader, model, ema_model, criterion, optimizer, scheduler, epoch):
    global args, step
    losses = AverageMeter()
    top1 = AverageMeter()
    model.train()
    ...
    for index, (input, target) in enumerate(train_loader):
        ...
        nn.MSELoss()(F.softmax(output, 1), F.softmax(ema_output, 1)) / input.shape[0]
        ...
        prec1 = accuracy(ema_output, target, topk=(1,))
        losses.update(loss.item(), input.shape[0])
        top1.update(prec1[0].item(), input.shape[0])
        if (index + 1) % int(len(train_loader) / 3 - 1) == 0:
